...
import { CreateLoanDto, LoanTrackingResponseDto, CreateLoanResponseDto } from './dto';
...
export class LoansController {
  ...
  @Get('tracking')
  @Public()
  @ApiOperation({ summary: 'Obtener información del préstamo por DNI y código de tracking (Endpoint público)' })
  @ApiQuery({ name: 'dni', description: 'Número de DNI del cliente', example: '12345678' })
  @ApiQuery({ name: 'tracking', description: 'Código de tracking del préstamo', example: 'LOAN-2024-001' })
  @ApiResponse({ 
    status: 200, 
    description: 'Información del préstamo obtenida exitosamente',
    type: LoanTrackingResponseDto 
  })
  @ApiResponse({ status: 400, description: 'Solicitud incorrecta - Parámetros faltantes' })
  @ApiResponse({ status: 404, description: 'Préstamo no encontrado o DNI no coincide' })
  async getLoanByTracking(
    @Query('dni') dni: string,
    @Query('tracking') tracking: string,
  ) {
          if (!dni || !tracking) {
        throw new BadRequestException('Se requieren tanto el DNI como el código de tracking');
      }
  ...
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Obtener todos los préstamos del usuario autenticado' })
